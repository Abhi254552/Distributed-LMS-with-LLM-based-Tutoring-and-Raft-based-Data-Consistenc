...
class RaftService(lms_pb2_grpc.RaftServiceServicer):
    ...
    def AppendEntries(self, request, context):
     ...
     # Step 2: Update term and leader information if leader's term is higher
     if request.leader.term > self.term or self.state != "Follower":
         self.step_down(request.leader.term, leader_id=request.leader.leaderID)
    ...
    def step_down(self, new_term, leader_id=None):
     """
     Transition from Leader or Candidate to Follower and update the term.
 
     Args:
         new_term (int): The new term to update to.
         leader_id (int, optional): The ID of the new leader (if known).
     """
     if self.term < new_term:
         print(f"Server {self.server_id}: Stepping down. Previous term: {self.term}, New term: {new_term}")
         self.term = new_term  # Update to the new higher term
     else:
         print(f"Server {self.server_id}: Already at term {self.term}. No need to update to {new_term}.")
 
     self.state = "Follower"  # Transition to Follower
     self.voted_for = None    # Reset voted_for to allow new voting in the new term
     self.leader_id = leader_id  # Update leader ID if provided
     self.timer = 0           # Reset election timer to avoid immediate election timeout
 
     # Log the role change for debugging purposes
     print(f"Server {self.server_id} is now a Follower in term {self.term}. Leader: {self.leader_id}. Election timer reset.")
...
# New GradeAssignment method
    ...
    SERVER_ADDRESSES = {
        1: "172.17.82.219:50051",
        2: "172.17.83.73:50052",
        3: "172.17.83.208:50053",
        4: "172.17.48.248:50054",
        5: "172.17.82.1:50055"
    }
...
if __name__ == "__main__":
    # Define configurations for all servers with new IPs
    server_configs = {
        1: {"port": 50051, "peers": {2: "172.17.83.73:50052", 3: "172.17.83.208:50053", 4: "172.17.48.248:50054", 5: "172.17.82.1:50055"}},
        2: {"port": 50052, "peers": {1: "172.17.82.219:50051", 3: "172.17.83.208:50053", 4: "172.17.48.248:50054", 5: "172.17.82.1:50055"}},
        3: {"port": 50053, "peers": {1: "172.17.82.219:50051", 2: "172.17.83.73:50052", 4: "172.17.48.248:50054", 5: "172.17.82.1:50055"}},
        4: {"port": 50054, "peers": {1: "172.17.82.219:50051", 2: "172.17.83.73:50052", 3: "172.17.83.208:50053", 5: "172.17.82.1:50055"}},
        5: {"port": 50055, "peers": {1: "172.17.82.219:50051", 2: "172.17.83.73:50052", 3: "172.17.83.208:50053", 4: "172.17.48.248:50054"}},
    }

    # Specify the server ID manually (e.g., Server 1)
    # Change this ID for each server instance you are running
    server_id = 1  # Change this value (e.g., 1, 2, 3, 4, 5)

    # Get the configuration for the specified server ID
    if server_id in server_configs:
        config = server_configs[server_id]
        serve(server_id, config["port"], config["peers"])
    else:
