...
How It Works
User Authentication: Students and instructors log in via the GUI client.
Assignment Workflow: Students upload assignments (PDFs); instructors download, grade, and provide feedback.
Course Material Sharing: Instructors upload course materials for students to access.
Query System: Students can ask questions, choosing between LLM or instructor responses.
...
Data Consistency: All critical operations (e.g., grading) are replicated across all LMS servers using Raft, ensuring consistency and fault tolerance.
Leader Election: If the leader server fails, Raft automatically elects a new leader, and the system continues without data loss.
Demo Video
...
Run LMS Servers (on separate machines or terminals)
